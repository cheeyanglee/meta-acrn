...
# loop through all rootfs image, remove from the size if it exists, as its not installed to /boot
for rootfs_img in /run/media/$1/rootfs*.img; do
  boot_size=$(( boot_size - $( du -ms $rootfs_img | awk '{print $1}') ))
done
...
#loop through all rootfs image, install them one by one
for rootfs_img in /run/media/$1/rootfs*.img ; do
  # Get a list of available hard drives
  ...
  for device in $devices; do
      #skip device used in previous installation
      for used_device in $USED_DEVICES; do
        if [[ "$device" == "$used_device" ]]; then
          continue 2
        fi
      done
  ...
  while [ -z "$TARGET_DEVICE_NAME" ];do
      echo "Please select an install target for $rootfs_img or press n to exit ($hdnamelist ): "
      read answer
      if [ "$answer" = "n" ]; then
      ...
      for hdname in $hdnamelist; do
          if [ "$answer" = "$hdname" ]; then
              ...
              USED_DEVICES="$USED_DEVICES $answer"
              break
          fi
      done
  done
  ...
  if [ -n "$TARGET_DEVICE_NAME" ]; then
      echo "Installing image $rootfs_img on /dev/$TARGET_DEVICE_NAME ..."
  else
  ...
  mount -o rw,loop,noatime,nodiratime $rootfs_img /src_root
