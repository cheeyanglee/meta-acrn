...
class BootimgEFIPlugin(SourcePlugin):
    ...
    def do_configure_partition(cls, part, source_params, creator, cr_workdir,
        ...
        try:
            if source_params['loader'] == 'grub-efi':
                ...
                raise WicError("unrecognized boot loader: %s" % source_params['loader'])
        except KeyError:
            raise WicError("requires a loader, none specified")
    ...
    def do_prepare_partition(cls, part, source_params, creator, cr_workdir,
        ...
        # copy acrn efi
        cp_cmd = "cp %s/acrn.efi %s/EFI/BOOT/bootx64.efi" % (kernel_dir, hdddir)
        exec_cmd(cp_cmd, True)
        ...
        try:
            if source_params['loader'] == 'grub-efi':
                ...
                for mod in [x for x in os.listdir(kernel_dir) if x.startswith("grub-efi-")]:
                    cp_cmd = "cp %s/%s %s/EFI/BOOT/native-bootloader.efi" % (kernel_dir, mod, hdddir)
                    exec_cmd(cp_cmd, True)
